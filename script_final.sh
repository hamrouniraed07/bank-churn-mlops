...
# Configuration du registre pour l'environnement
echo "Configuration du registre ACR pour l'environnement..."
az containerapp registry set \
  --name "$CONTAINER_APP_NAME" \
  --resource-group "$RESOURCE_GROUP" \
  --server "$ACR_LOGIN_SERVER" \
  --username "$ACR_USER" \
  --password "$ACR_PASS" 2>/dev/null || true
...
if az containerapp show -n "$CONTAINER_APP_NAME" -g "$RESOURCE_GROUP" >/dev/null 2>&1; then
  ...
  # Configurer les credentials après création
  echo "Configuration des credentials ACR..."
  az containerapp registry set \
    --name "$CONTAINER_APP_NAME" \
    --resource-group "$RESOURCE_GROUP" \
    --server "$ACR_LOGIN_SERVER" \
